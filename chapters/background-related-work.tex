...
\begin{example}\label{ex:bm}
  ...
  \[
    \Hcal(K_{\text{bm}}) = \left\{f: f \text{ is absolutely continuous, } f(0)=0 \text{ and } f'\in L^2[0, 1]\right\},
  \]
  with inner product \(\dotprod{f}{g}_{K_{\text{bm}}} = \text{\large\(\int\)}f'g'\).
\end{example}
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=.2\textwidth]{cube_sphere}
...
\end{enumerate}
 \begin{figure}[ht!]
   \centering
   \includegraphics[width=.5\textwidth]{mh}
