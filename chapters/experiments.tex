...
\begin{enumerate}
  ...
  \item A fractional Brownian motion \(K_2(t,s)=1/2(s^{2H} + t^{2H} - |t-s|^{2H})\), with Hurst parameter \(H=0.8\).
...
\paragraph{Logistic regression data sets.} Again we consider three different types of data sets,  with a common value of \(\alpha_0=-0.5\).
\begin{itemize}
...
In this section we present some experiments that are not directly related to measuring the prediction performance relative to other methods.
...
\incomment{Me gustaría añadir aquí un último experimento en el que se muestran gráficas de evolución del RMSE o del Accuracy según va variando alguno de los hiperparámetros (especialmente el valor de p y el número de ejemplos de entrenamiento).}
